...
H=800; W=640;
dataloaders = {
    'train': DataLoader(dataload(path='data/train', H=H, W=W,pow_n=8, aug=True) , batch_size=batch_size, shuffle=True, num_workers=7),
...
def HtoA(x):
    ...
    x = x/(x.max(dim=1).values.unsqueeze(1)+(1e-100)) # x.max(dim=1) == size 19 * 1
    x = torch.pow(x,5)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('model/0823/LOSS = axisloss/Network0825_0.00033906658468670986_E_1189.pth',map_location=device_txt))
    ...
    for epoch in range(num_epochs):
        ...
        for phase in phase_:
            ...
            # deep copy the model
            savepath = 'model/0823/LOSS = axisloss/Network0825_{}_E_{}.pth'
