...
import random
from a1net_ import *
import a1net_ as network

#os.chdir("/content/drive/MyDrive/ISBI_pytorch/ISBI_pytorch")
batch_size = 1
H=800; W=640;
dataloaders = {
    'train': DataLoader(dataload(path='data/train', H=H, W=W,pow_n=8, aug=True) , batch_size=batch_size, shuffle=True, num_workers=5),
    'val': DataLoader(dataload(path='data/test1', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False, num_workers=5)
}
...
if __name__ == '__main__':
    ...
    model=network.SEU_Net(1, num_class).to(device)
    #model = network.ECAU_Net(1, num_class).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in uu:
            ...
            print(phase,"Joint loss :", epoch_Jointloss.item() )

            # deep copy the model

            savepath = 'E:\X-Ray\model\isbi/Network_{}_E_{}.pth'
