...
dataloaders = {
    ...
    'val': DataLoader(dataload(path='data/test1', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False, num_workers=10)
}
...
def HtoA(x):
    ...
    x = x/(x.max(dim=1).values.unsqueeze(1)+1e-17) # x.max(dim=1) == size 19 * 1
    x = torch.pow(x,5)
        ...
        for phase in phase_:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    ## loss compute
                    regressloss, axisloss  = L2_loss(outputs, labels), L2_loss(Axis_Pred, Axis_GT)*1e-10
                    LOSS = axisloss # regressloss+
            ...
            # deep copy the model
            savepath = 'model/0823/LOSS = axisloss/Network0820_{}_E_{}.pth'
