...
H=800; W=640;
dataloaders = {
    'train': DataLoader(dataload(path='data/train', H=H, W=W,pow_n=8, aug=True) , batch_size=batch_size, shuffle=True, num_workers=6),
    'val': DataLoader(dataload(path='data/test1', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False, num_workers=6)
}
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('model/21_10_06/Network_0.0016354945255443454_E_659.pth',map_location=device_txt))

    # Observe that all parameters are being optimized
    num_epochs = 2000
    # optimizer = Adam(lr = 1e-3, weight_decay=0.1)
    optimizer = optim.Adam(model.parameters(), lr=1e-4)
    # 어느정도 학습 되면 lr_scheduler -> Custom CosineAnnealingWarmRestarts로 변환
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, num_epochs, 1e-7)
    print("****************************GPU : ", device)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in uu:
            ...
            savepath = r'E:\X-Ray\model\22_02_22_3py\Network_{}_E_{}.pth'
