...
H=800; W=640;
dataloaders = {
    'test1': DataLoader(dataload(path='data/test1', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False, num_workers=3),
    'test2': DataLoader(dataload(path='data/test2', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False, num_workers=3)
}
    ...
    for phase in phase_:
        ...
        for inputs, labels in dataloaders[phase]:
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                Axis_Pred[:, 0], Axis_Pred[:, 1]  = Axis_Pred[:, 0] * (2400 / H), Axis_Pred[:,1]*(1935 / W)
                Axis_GT[:, 0], Axis_GT[:, 1] = Axis_GT[:, 0] * (2400 / H), Axis_GT[:, 1] * (1935 / W)

                print('num', num_, 'LOSS ', LOSS, 'AxisLoss' , axisloss, '#EDIST', edist_loss(Axis_Pred, Axis_GT)  )
                # if num_%100==0:
