...
def ACloss(pred, target):
    L12loss = torch.mean(torch.pow((pred - target), 2))
    #L12loss =torch.mean(torch.abs(pred - target))
    if batch_size>1:
        ang_loss = 0; dist_loss = 0;
    ...
    w_loss = (1+(ang_loss)) + log(dist_loss)
    metrics['L12loss'] += L12loss.data.cpu().numpy() * target.size(0)
    metrics['distloss']+= dist_loss # 필요없음 배치 사이즈 만큼 곱하기
    #loss = torch.mul(L12loss , w_loss)
    loss = L12loss

    metrics['reconloss']+= loss.data.cpu().numpy() * target.size(0)
    return loss

criterion = nn.CrossEntropyLoss()
def cls_loss(pred, target):

    l1=criterion(pred[0:1,:], target[:,0])
    l2 = criterion(pred[1:2, :], target[:, 1])
    l3 = criterion(pred[2:3, :], target[:, 2])
    l4 = criterion(pred[3:4, :], target[:, 3])
    l5 = criterion(pred[4:5, :], target[:, 4])
    l6 = criterion(pred[5:6, :], target[:, 5])
    l7 = criterion(pred[6:7, :], target[:, 6])
    l8 = criterion(pred[7:8, :], target[:, 7])

    loss=l1+l2+l3+l4+l5+l6+l7+l8

    metrics['clsloss'] += loss * target.size(0)
    return loss
...
if __name__ == '__main__':
    ...
    #model = network.MT_Net().to(device)
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, 700)
    print("****************************GPU : ", device)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in uu:
            ...
            savepath = 'model/h2/3way_1e2_simplesum_attunet_{}_L1_{}_E_{}.pth'
