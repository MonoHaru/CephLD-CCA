...
dataloaders = {
    'train': DataLoader(dataload(path='data/train', H=H, W=W, pow_n=8, aug=True), batch_size=batch_size, shuffle=True,
                        num_workers=8),
    'val': DataLoader(dataload(path='data/test1', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False,
                      num_workers=8),
    'test': DataLoader(dataload(path='data/test2', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False,
                       num_workers=8)
}
...
if __name__ == '__main__':
    ...
    model = network.my_Net_01(1, num_class).to(device)
    print(model)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in uu:
            # since = time.time()
            if phase == 'train':
                scheduler.step(); model.train()  # Set model to training mode
            else:
            ...
            savepath1 = r'E:\X-Ray\model\Cartesian_Squeeze_and_Excitation_U_Net_003\Network_{}_E_{}.pth'
            savepath2 = r'E:\X-Ray\model\Cartesian_Squeeze_and_Excitation_U_Net_003\Best_Network_{}_E_{}.pth'
    ...
    plt.title('Cartesian_Squeeze_and_Excitation_U_Net_003')
    plt.xlabel('epoch')
    plt.ylabel('loss')
