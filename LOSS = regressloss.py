...
H=800; W=640;
dataloaders = {
    'train': DataLoader(dataload(path='data/train', H=H, W=W,pow_n=8, aug=True) , batch_size=batch_size, shuffle=True, num_workers=10),
    'val': DataLoader(dataload(path='data/test1', H=H, W=W, pow_n=8, aug=False), batch_size=batch_size, shuffle=False, num_workers=7)
}
...
if __name__ == '__main__':
    ...
    #model=torch.load('model/0823/LOSS = regressloss/Network0816_0.00037229315717316543_E_939.pth').to(device)
    model=network.AttU_Net(1, num_class).to(device)
    model.load_state_dict(torch.load('model/0823/LOSS = regressloss/Network0825_0.0005681738156514863_E_359.pth',map_location=device_txt))
        ...
        for phase in phase_:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    ## loss compute
                    regressloss, axisloss  = L2_loss(outputs, labels), L2_loss(Axis_Pred, Axis_GT)*1e-8
                    LOSS = regressloss+axisloss + 1e-50
                    ...
                    metrics['regressloss'] += regressloss.item()
                    metrics['axisloss'] += axisloss.item()
            ...
            print(phase,"Joint loss :", epoch_Jointloss, "regress loss :", epoch_regressloss)
            print("axis loss :", epoch_axisloss, 'edist', epoch_edist.item() )

            # deep copy the model
            savepath = 'model/0823/LOSS = regressloss/Network0903_{}_E_{}.pth'
