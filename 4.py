...
H=1200; W=960;
dataloaders = {
    'train': DataLoader(dataload(path='data/train', H=H, W=W,pow_n=8, aug=True) , batch_size=batch_size, shuffle=True, num_workers=8),
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(r'E:\X-Ray\model\1_4py\Network_0.00558353029191494_E_29.pth',map_location=device_txt))

    # Observe that all parameters are being optimized
    num_epochs = 2000
    # optimizer = optim.Adam(model.parameters(), lr=1e-5)
    optimizer = optim.Adam(model.parameters(), lr=9.99200775054487e-06)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in uu:
            ...
            if phase == 'train':
                train_losses.append(epoch_Jointloss_cpu)
            else:
                val_losses.append(epoch_Jointloss_cpu)

            print(phase,"Joint loss :", epoch_Jointloss )
            # deep copy the model

            savepath = r'E:\X-Ray\model\1_4py\Network_{}_E_{}.pth'
